...
- **Order-level fees omitted from invoices**: WooCommerce order fees — surcharges (gift wrap, COD/payment fees) and order-level discounts applied as fees (store credit, loyalty programs, manual admin adjustments) — never appeared on the generated invoice. A fee is not part of any line's subtotal/total, so the invoice drifted from the order: surcharges under-reported the taxable base, fee discounts over-reported it. Each fee is now sent as a document-level allowance/charge (`allowance_charge_indicator` `allowance` for a discount, `charge` for a surcharge) carrying its own tax category, so it lands in the correct VAT bucket and the invoice reconciles with the order. Verified end-to-end against staging for standard invoices and Spanish rectificatives.
- **Credit notes / rectificatives under-reversed VAT**: the per-line tax rate was derived behind a positive-total guard, so a refund line — whose total is negative — resolved to 0% and was treated as tax-exempt. The line VAT was therefore never reversed and the credit-note / rectificative total came out wrong (the tax was simply missing). The rate is now taken from the absolute values, so refund lines keep their original rate. Verified against staging: an ES rectificative now totals the exact negative of the original invoice.
...
- **Vendor Cleanup in Release ZIP**: The B2Brouter PHP SDK package shipped in `vendor/` carried 44 development-only files (`.env.example`, `.gitignore`, `.github/`, `phpunit.xml.dist`, the package's own `tests/`, `docs/`, `examples/` directories, `CHANGELOG.md`, and `README.md`). Composer's `--no-dev` flag strips dev *dependencies* but does not strip dev *files within prod packages*. `build-release.sh` now removes these patterns from `${RELEASE_DIR}/vendor` after `composer install`, and a follow-up `find` re-scan fails the build if any known dev pattern remains. The shipped ZIP drops from 75 to 31 vendor files. Surfaced by the WordPress.org Plugin Review Team flagging `.env.example` as a not-permitted file
...
[1.0.6]: https://github.com/B2Brouter/b2brouter-woocommerce/releases/tag/v1.0.6
